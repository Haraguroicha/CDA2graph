...
## 第一節 研究背景
國內衛生署推行的電子病歷是由HL7所制定的臨床文件架構所制定的一種XML文件，這文件格式現在在國內已經有數百家醫院使用，各個醫院必須透過各自的ECC Gateway，經過健保局提供的VPN網路，與衛生署電子病歷交換平台(EEC, E.M.R. Exchange Center, Electornic Medical Record Exchange Center)交換電子病歷資料。各醫院的醫師看診時，若要看其他醫院所產出的電子病歷，則必須在有病患的健保卡與醫師的醫事憑證卡，連線至EEC平台申請相關的電子病歷資料調閱，隨後等待資料回傳後，即可直接在HIS中直接瀏覽資料。但目前交換電子病歷資料的對等醫院皆必須要有HIS系統能夠編、解碼，否則交換後的資料無法解讀給醫生瀏覽。至目前為止，國內所有的電子病歷都必須要各家醫療院所自行開發電子病歷相關的編、解碼程式，然後經過衛生署認證後，才能上線至EEC平台開始做電子病歷交換的流程。
...
ITIL Version 2以前，主要分成兩個部份：服務支援 (Service Support) 與服務遞送 (Service Delivery)。ITIL 最新版本(Version 3)則將安全管理(Security Management)納入考量，所以ITIL包含七個模組：
...
### 貳 服務管理
其實要述說IT服務管理，倒不如將熟悉的傳統IT管理的理念與現在IT服務管理觀念有甚麼轉變做個比較會比較易懂(圖2-11 傳統IT管理與IT服務管理)。

圖 2 11 傳統IT管理與IT服務管理
資料來源: Vernon Lloyd, etc.. Planning to implement Service Management. OGC, 2002.

接下來，將前面所述ITIL第一個模組服務管理(ITSM, IT Service Management)內細分的服務支援(Service Support)及服務遞送(Service Delivery)兩部分細說如下：

#### 服務支援(Service Support)
服務支援的範圍有五種子管理：事件管理(Incident Management)、問題管理(Problem Management)、變更管理(Change Management)、組態管理(Configuration Management)、上線/發行管理(Release Management)。

在客服中心的值機台，一般常發生的管理流程有二步驟：

1. 對異常回報的確認、記錄與分類，並給予最初支援
2. 後端支援人員判斷此異常狀況屬於事件(incident)或問題(problem)

事件的發生並不影響客戶使用的電信設備或是服務，例如狀況的查詢(帳單、申請)；但是問題的發生確會影響客戶對電信設備的使用及服務，因此問題的處理往往牽涉到客服後台支援和持續的追蹤。

事件管理(Incident Management)有四個處理流程階段：
...
問題管理(Problem Management)有七個處理流程階段：
...
然而對電信客服管理，因為用戶要求變更的通常與通話服務功能、用戶端通信(非IT)設備或是用戶註冊資料變動有關；換句話說，ITIL是對IT部門如何管理所提供的服務，其中包括提供IT設備給IT使用者；而電信客服所提供的是協助用戶解決使用電信設備(並非客服中心的IT設備)的問題，所以ITSM所說的變更管理、組態管理、上線/發行管理的部分流程，並不完全適用於電信客服流程，而需稍加修改才符合電信客服之需求。

變更管理(Change Management)有八個處理流程階段：本來「變更管理」的目的是確保IT設計或流程變更後的服務品質。電信設備或是服務應用軟體(有可能是服務供應商提供)相關的變更管理也是一樣的目的；只不過前者是以IT部門設計的軟體或建置的硬體為主，後者則比較複雜，它包括電信網路製造商、服務供應商提供及服務整合(Service Integration)單位負責的軟/硬體。這些電信網路製造商、服務供應商或服務應用軟體系統整合單位，他們負責測試及實行，客服中心則根據工程單位安排時程，並配合測試部分相關變更的客服流程，通過測試後才實行商業運轉。不過更嚴謹的是，電信設備是24小時不間斷執行；換言之，電信設備軟/硬體的變更管理，凡牽涉系統變更，必須不影響(不可中斷)客戶通話，所以必須線上(online)做變更，而且要準備一套還原(roll-back)步驟，以便變更失敗時，在最短時間內恢復變更前的原狀，以不損害客戶權利為原則，所以有下列流程：

1. 提出在實驗室已證明線上變更可行性無誤的變更計畫流程(有時變更計畫是由合作夥伴提出並要求更改作業流程)
2. 註冊及過濾變更、決定變更要求的優先順序
3. 將變更要求依影響的程度做分類
4. 召開CAB (Change Advisory Board)會議，評估影響及配置資源
5. 核准變更申請
6. 依預先安排的時程，進行變更建立、測試及實行
7. 依預定的目標，審查變更的結果(變更有誤：還原)
8. 管理變更資料(變更無誤)

組態管理(Configuration Management)有五個處理流程階段：組態管理是負責記錄電信設備或是服務應用軟體的屬性/功能，以及實體的數量，所以它的資訊必須是精準的，任何服務支援或服務管理，都要基於組態管理的資料庫，例如對於一個突如其來的災難地區，它的電(鏈)路的調配，如果對電信設備或是服務應用軟體的屬性/功能，以及實體的數量，都沒有精確的紀錄，臨時突發事件，想要調配電路都不知所措，不是界面屬性不符無法調配，就是延誤時機，造成客戶的抱怨，所以組態管理大致可分為下列五大流程階段執行：

1. 規劃 (Planning)	定義範疇，目標，政策，角色職責以及組態管理的流程
2. 定義(Identification)	詳細指明與定義環境下的網路元件、鏈路與整體CMDB的設計架構
3. 控制 (Control)	妥善維護，唯有透過適當的變更流程進行網路元件、鏈路及CMDB的更新
4. 狀態描述(Status Accounting)	所有關於網路元件、鏈路及CMDB中各細目之當下與歷史紀錄
5. 確認與稽核(Verification & Audit)	藉由一連串的稽核確保網路元件、鏈路及CMDB中各細目屬性, 彼此連結的一致性

上線/發行管理(Release Management)有六個處理流程階段：一般IT「上線/發行管理」與電信設備或是服務應用軟體(有可能是服務供應商提供)相關的上線/發行管理是一樣的目的；只不過前者是以IT部門設計的軟體或建置的硬體為主，後者則比較複雜，它包括電信網路製造商、服務供應商提供及服務整合(Service Integration)單位負責的軟/硬體。這些電信網路製造商、服務供應商或服務應用軟體系統整合單位，他們負責測試及實行，客服中心則根據工程單位安排時程，並配合測試部分相關上線/發行的客服流程，通過測試後才實行商業運轉。不過更嚴謹的是，電信設備是24小時不間斷執行；換言之，電信設備軟/硬體的上線/發行管理，凡牽涉系統變更，必須不影響(不可中斷)客戶通話，所以必須線上(online)做上線/發行，而且要準備一套還原(roll-back)步驟，以便上線/發行失敗時，在最短時間內恢復上線/發行前的原狀，以不損害客戶權利為原則；換句話說，管理所有電信服務的變更執行流程，確保所有的變更，均受到適當的管控，並符合服務等級協定(SLA)的要求，其目標為：電信設備或是服務應用軟體的上線/發行管理需要嚴密的監督過程，雖然這些活動都屬於電信網路工程單位負責，但是若與客服中心相關作業流程有關，則應同步處理，所以有下列流程：

* 透過變更管理流程，確認所要執行的工作(流程)
* 規劃與監督軟體與相關硬體的上線；過程中，需與客戶進行協調與溝通，以不影響客戶日常作業為優先考量。
* 設計與發展有效率的流程，進行電信系統或用戶建置流程的變更
* 確保執行變更的軟硬體能被追蹤，且唯有經過認可、安全，測試通過的版本能被發佈，以及相配合下模組資料庫的更新
...
* 上線管理者
	* 審視服務變更需求
	* 組織並促進變更顧問委員會議的進行
	* 上線流程切割成數個邏輯單位，提升變更協調者的處理效率
	* 準備一個案說明以爭取新服務的開發基金
	* 帶動整個上線流程，並適時矯正錯誤
	* 上線後，仍必須與相關人員協商、開會，蒐集對於日後的上線計畫的建議
* 變更顧問委員會
	* 對於所提列的變更需求單進行審核 (拒絕、執行、再次審查)

#### 服務傳遞
服務遞送有以下五種管理：容量管理(Capacity Management)、服務水準管理(Service Level Management)、可用性管理(Availability Management)、服務永續性管理(Service Continuity Management)、服務的財務管理(Financial Management for Service)。同樣的，ITSM這些管理都針對IT的服務管理，而不是針對電信用戶所使用的網路設備，因此必須將ITSM所描述的管理設備轉為電信設備，然後配合各電信用戶及電信網路營運商的需求修改後做為客服中心執行。

容量管理(Capacity Management)有3個子流程階段：不當的IT容量規劃可能造成資源浪費，導致不必要的成本，或者資源缺乏則可能導致效能不佳。電信用戶不像IT的使用者，前者人數可以多到超過百萬，而且可能每日數以萬計的用戶急遽的上升，其有如螞蟻雄兵一樣的可怕。所以沒有良好的容量管理，這些急遽上升的用戶會衝擊並造成整個電信系統癱瘓。容量管理是透過最佳化服務成本，安排採購時間以及部署網路資源，從而對顧客協議之履行提供支援的流程。容量管理主要涉及資源管理、性能管理、需求管理、模擬測試、能力規劃、負載管理以及服務應用選型。換言之，容量管理主要是透過關鍵需求的規劃和整合，從而確保約定的服務級別可以實現的一種過程。

* 目的：
	* 長期：規劃及設計電信/服務應用/IT/客服網路於適當時期擴充，避免客戶需要服務時，無法配置使用。
	* 緊急：延緩/控制客戶緊急大量使用，避免電信/服務應用/IT/客服系統超載，造成系統當機。
* 三個子流程
* 企業容量管理(BCM)
	* 目標-了解未來電信/服務應用/客服網路用戶的需求容量
	* 從企業客戶那蒐集資訊，例如：策略計畫或趨勢分析。這個子流程是屬於前測的，和服務水準管理有很大的關係
* 服務容量管理(SCM)
	* 目標-決定並了解電信/服務應用/IT/客服尖峰時刻的使用 
	* 了解尖峰時刻的績效以確保簽定適當的服務協定
* 資源容量管理(RCM)
	* 目標-決定並了解電信/服務應用/IT/客服基礎建設容量
	* 主動的偵測潛在問題可以幫助資源有效的利用。資源包括了主機/伺服器/路由器/網路(接取、傳輸或交換)元件的容積、網路(鏈路、路徑)頻寬、流程和磁碟容量等

服務水準管理(Service Level Management)：

* 目標：
	* 要與顧客就所要提供之電信、服務應用的類型和品質簽訂清晰的協議。
		* 服務人員更了解客戶(組織內/外)的期望，利用服務流程確保能夠實現期望
		* 依客戶需要“量身訂做”提供適當的能力與服務
		* 提高了客戶滿意度
		* 網路價值得到更好的體會，增進人員的士氣與工作滿意度
		* 提高了服務的靈活性與可適應性
	* 要電信網路工程/行銷/業務部門/財務/客服部門確保這些協議得以實施。
		* 確保資訊/安裝/工程作業流程、支持業務流程，進而提升業務營運的質量
		* 與相關單位流程化的溝通互動，提高預知未來趨勢的能力
		* 改進IT的支持與服務，使電信網路工程/行銷/業務部門/財務/客服部門能更加靈活的使用IT

又，協議的種類科分為三種：

* 服務水準協定(Service Level Agreement, SLA)	說明了主要的服務目標與雙方的責任，去實行顧客合理的服務期望，建立電信服務業者與客戶間雙向的責任關係。
* 作業層級協定(Operating Level Agreement, OLA)	建立由客服支援維護團隊要遞交給客服服務部門中特別的技術、資訊、時間框架(timeframe)的需求。
* 服務支撐合約(Underpinning Contract, UC)	與第三方供應商簽訂供應鏈當中的責任歸屬

因此，服務級別管理需要有關顧客需求、電信網路工程部門、行銷部門、業務部門、資訊部門可提供之設施，以及財務資源等方面的資訊。

可用性管理(Availability Management)有兩個處理流程階段：

* 目標：
	* 透過適當資源、方法和技術的部署，從而確保與顧客約定之電信網路及服務應用(SA)可用性目標得以實現的流程。
	* 主要涉及維護活動的最佳化規劃，以及設計相關措施，將事件發生的次數降至最低。

階段一：規劃(Planning)：由於此部分牽涉電信網路、服務應用軟體(內製、委外或Ad hoc夥伴)及IT的規劃，所以服務經理必須參加三種不同特性的規劃，不僅反映客服需求，同時也要求各單位(包含內部、委外、夥伴)設計「可用性」及「復原」的監督績效評核準則，才有辦法制定客服管理方法。
...
* 監督和趨勢分析
* 檢閱服務(服務應用/IT/客服)及設備(電信網路元件)的可用性

服務永續管理(Service Continuity Management)：服務持續性管理處理程序是指當服務發生中斷後：
...
* 電信服務持續性管理之主題在於管理”風險”
	* 確保組織的電信網路及服務應用軟體基礎架構，在發生不可能發生的事件或非預期的事件時可以繼續提供服務
	* 即確保當發生正常可用性解決方案失敗的事件時，仍能夠提供價值給客戶
* 透過商業處理程序的分析，對於電信網路及服務應用軟體基礎架構弱點進行風險控管
* 持續性管理應結合於企業整體永續經營計畫中

服務的財務管理(Financial Management for Service)：電信網路、服務應用系統(包含內製、委外、Ad hoc服務)、資訊系統以及客服之間的績效關係較難衡量，但若實施財務管理程序，則可產生各項活動成本的能見度，增加規劃、最佳化及成本回收的益處，亦即：

* 藉由估算執行該客服(內製、委外及Ad hoc服務)成本，以提供顧客合理的價格
* 藉由合理的拆帳，鼓勵委外之服務應用系統創新，以減低電信業者為不斷提供創新服務的額外成本。
* 導入「使用者付費」觀念，進而改變客戶濫用資源的惡習
