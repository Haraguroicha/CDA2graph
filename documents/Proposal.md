...
其中，在衛生署的計畫中指出，至目前為止，推動的工作年度目標中，實際通過檢查於查驗的成果應該有如下表所示：

|檢查項目|通過檢查醫院數量|查驗項目|通過查驗醫院數量|
|-------|-------------|-------|-------------|
|X光|N家|醫療影像及報告|N家|
...
1. 醫事單位開發界面
	...
	在眾多使用者與醫療單位的使用之下，必定會有電子病歷呈現的問題，這需要有樣板檔的設計才有辦法呈現，故需要一個提供醫事單位開發及設計的界面，方便醫事單位可以將客制化的表單設計出來使用。這個開發界面會結合模板編輯器以及檔案解析介面的系統功能，在醫事單位開發的流程上就會有如下圖所示：
	
	(這是圖要畫的流程)匯入要開發的單張 > 解析單張所需的XPath > 利用IDE及所見即得模式來設計表單 > 存入資料庫
	
2. 樣板檔資料庫
	
	樣板檔資料庫內除了提供了醫事單位所設計的樣板檔之外，還包含了各種樣板檔所提供的用途清單和組合列表，在用途清單中會描述著這個樣板是用於畫面檢視還是物件的呈現，而組合列表則是會描述一個物件檢視中會使用到哪些畫面檢視元件和哪些物件的組合和呈現。詳細的描述結構有如下圖所示：
	
	(這是圖要畫的圖的結構)資料庫分為用途清單及組合列表，用途清單中描述著每個樣板的用途，組合清單則是特定用途的樣板會有描述該組合內會用到哪些樣板，依此類推。
	
3. 樣板檔組合電子病歷形成檢視
	
	在設計完成的樣板檔內會有存入該樣板檔可提供單張檢視的清單，這些樣板檔會在電子病歷讀入的時候自動的被載入，並且形成各區塊的病歷資料內容，一個電子病歷單張可能會結合一到多張的樣板檔來結合使用。因此，形成檢視的過程中，系統會有如下圖所示的流程：
	
	(這是圖要畫的流程)載入要檢視的病例單張 > 解析該單張所需的樣板檔 > 載入需要的樣板檔 > 將資料插入樣板檔模型內 > 顯示至使用者畫面上
	
4. 外部使用的API介面
	
	在醫事單位內，可以利用API介面來進行遠端的呼叫及引用系統功能，甚至可以整合進入HIS畫面中，節省開發時間。在使用API的時候可以當作嵌入式檢視器，也可以當作匯入電子病歷檔案至HIS資料庫的呼叫媒介，詳細API會提供的呼叫方式及功能結構如下圖所示：
	
	(這是圖要畫的圖的結構)API分為兩種，一種是嵌入式檢視器，一種是當作遠端呼叫，嵌入式檢視器僅提供檢視器的基本功能(即單純的檢視，不提供額外的互動，例如：互動圖表、詳細資料等檢視)，但會提供轉換至完整版的按鈕呼叫，遠端呼叫的功能則會提供萃取區段資料、匯入電子病歷等功能。
